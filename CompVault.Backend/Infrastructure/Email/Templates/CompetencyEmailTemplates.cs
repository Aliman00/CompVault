...
public static class CompetencyEmailTemplates
{
    ...
    public static EmailBody ExpiringSoonToManager(
        ...
        int daysUntil)
    {
        string encodedManagerName = WebUtility.HtmlEncode(managerName);
        string encodedEmployeeName = WebUtility.HtmlEncode(employeeName);
        string encodedCompetencyName = WebUtility.HtmlEncode(competencyName);
        return new(
            Subject: $"{employeeName} sitt kompetansebevis «{competencyName}» utløper om {daysUntil} dager",
            Html: $"""
                   <h2>Hei {encodedManagerName},</h2>
                   <p><strong>{encodedEmployeeName}</strong> sitt kompetansebevis <strong>{encodedCompetencyName}</strong> utløper <strong>{expiryDate:dd. MMMM yyyy}</strong> — om <strong>{daysUntil} dager</strong>.</p>
        ...
        DateTime expiryDate)
    {
        ...
        return new(
            ...
            Html: $"""
                   <h2>Hei {encodedEmployeeName},</h2>
                   <p>Ditt kompetansebevis <strong>{encodedCompetencyName}</strong> utløpte <strong>{expiryDate:dd. MMMM yyyy}</strong>.</p>
        ...
        DateTime expiryDate)
    {
        ...
        return new(
            ...
            Html: $"""
                   <h2>Hei {encodedManagerName},</h2>
                   <p><strong>{encodedEmployeeName}</strong> sitt kompetansebevis <strong>{encodedCompetencyName}</strong> utløpte <strong>{expiryDate:dd. MMMM yyyy}</strong>.</p>
